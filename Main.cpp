...
std::string wptext = ("XX"), scptext = "XX", brltext = "XX";
...
void SetupStyles() {
	...
	style->Colors[ImGuiCol_Text] = ImVec4(0.80f, 0.80f, 0.63f, 0.80f);
...
int main()
{
	...
	while (!done)
	{
		...
		ImGui::NewFrame();
		{
			ImGui::Begin(("Alcatraz.Solutions Script"));
			{
				ImGui::BeginGroup();
				SetupStyles();
				...
				ImGui::SliderFloat(("##Sensitivity"), &Sensitivity, 0.001, 1, ("%.3f"));
				ImGui::Text(("FOV"));
				...
				if (ImGui::Button((" 8x"), ImVec2(75, 20))) {
					Mod8X = true; ModHOLO = false; ModHAND = false; scptext = ("8x");
				}
				if (ImGui::Button((" Hand"), ImVec2(75, 20))) {
					ModHAND = true; Mod8X = false; ModHOLO = false; scptext = ("hand");
				}
				if (ImGui::Button((" Holo"), ImVec2(75, 20))) {
					ModHOLO = true; Mod8X = false; ModHAND = false; scptext = ("holo");
				}
				if (ImGui::Button((" Silence"), ImVec2(75, 20))) {
					ModSILEN = true; brltext = ("silent");
				}
				ImGui::Spacing();
				ImGui::Spacing();
